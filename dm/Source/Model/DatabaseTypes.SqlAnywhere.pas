unit DatabaseTypes.SqlAnywhere;
...
type
  TSqlAnywhereDatabaseType = class
  public
    class procedure LoadSqlAnywhereExpressions(AList: TStrings);
  end;
...
class procedure TSqlAnywhereDatabaseType.LoadSqlAnywhereExpressions(AList: TStrings);
begin
  ...
  AList.Values[SQL_CREATEINDEX] :=
    'CREATE{ <%IndexTyrpe%>} INDEX <%IndexName%> ON <%TableName%>'#13#10+
    '  (<%IndexLstFields%>)';
  ...
  AList.Values[SQL_RELATIONSHIPDELETEACTION] :=
    ',RESTRICT,CASCADE,"SET NULL","SET DEFAULT",RESTRICT';
